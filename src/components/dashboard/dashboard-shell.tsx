...
export function DashboardShell({ requiredRole, title, subtitle, sidebarItems, children, onUserLoaded }: DashboardShellProps) {
        ...
        const loadCurrentUser = async () => {
            ...
            try {
                ...
                if (me.systemRole !== requiredRole) {
                    router.replace(me.systemRole === "ADMIN" ? "/admin-dashboard" : "/user-dashboard");
                    return;
                }
    ...
    const dashboardHref = requiredRole === "ADMIN" ? "/admin-dashboard" : "/user-dashboard";
        ...
        return (
            <main className="min-h-screen bg-[radial-gradient(circle_at_top,_rgba(59,130,246,0.08),_transparent_30%),linear-gradient(180deg,#f8fbff_0%,#eef3f9_100%)] px-4 py-6 text-slate-900 sm:px-6 lg:px-8">
                <div className="mx-auto flex min-h-[calc(100vh-3rem)] max-w-[1600px] gap-6">
                    <aside className="hidden w-80 flex-col gap-6 rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm lg:flex">
    ...
    return (
        <main className="min-h-screen bg-[radial-gradient(circle_at_top,_rgba(59,130,246,0.08),_transparent_30%),linear-gradient(180deg,#f8fbff_0%,#eef3f9_100%)] px-4 py-6 text-slate-900 sm:px-6 lg:px-8">
            <div className="mx-auto flex min-h-[calc(100vh-3rem)] max-w-[1600px] gap-6">
                <aside className={`hidden flex-col gap-6 rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm transition-all duration-300 lg:flex lg:sticky lg:top-6 lg:h-[calc(100vh-3rem)] ${compactSidebar ? "lg:w-24" : "lg:w-80"}`}>
                    <div className={`flex ${compactSidebar ? "flex-col items-center gap-4" : "items-start justify-between gap-4"}`}>
                    ...
                    <div className={`rounded-3xl border border-slate-200 bg-slate-50 p-4 ${compactSidebar ? "text-center" : ""}`}>
                        <p className="text-xs uppercase tracking-[0.28em] text-slate-500">Rôle actif</p>
                    ...
                    <nav className="space-y-2">
                        {sidebarItems.map((item, index) => (
                            <a
                                key={item.id || index}
                                href={item.href}
                                className="group flex items-start gap-4 rounded-3xl border border-slate-200 bg-white px-4 py-4 transition hover:-translate-y-0.5 hover:border-sky-200 hover:shadow-md hover:shadow-sky-100/60"
                            >
                                <span className={`mt-1 h-3 w-3 rounded-full border ${toneClasses[item.tone]}`} />
                                {!compactSidebar ? (
